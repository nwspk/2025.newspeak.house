	...
	<section id="rankings" class="rankings-section">
		...
		<div id="assessment-log" class="logs-stack">
			...
			<div class="tab-info" style="border-left-color:hsl(var(--chart-5))">
				<p class="tab-info-text">
					Each iteration's top projects with their assessments. Earlier iterations used scoring heuristics only — assessments marked * were inferred from the heuristic description rather than generated by a jury or agent.
				</p>
			</div>

			<div class="assessment-log-wrap">
				{#each versions as ver, vi}
					{@const iterProjects = (resultsMap[ver.version] ?? []).slice(0, 10)}
					{@const hasSynthetic = iterProjects.some(p => p.assessment_synthetic)}
					{@const hasReal = iterProjects.some(p => p.assessment && !p.assessment_synthetic)}
					<div class="alog-row" class:alog-row--expanded={assessmentLogExpanded === ver.version}>
						<button
							class="alog-header"
							type="button"
							onclick={() => assessmentLogExpanded = assessmentLogExpanded === ver.version ? null : ver.version}
							style="border-left-color:hsl(var(--{chartColors[vi % chartColors.length]}))"
						>
							<span class="alog-ver" style="color:hsl(var(--{chartColors[vi % chartColors.length]}))">{ver.version}</span>
							<span class="alog-title">{ver.title}</span>
							<span class="alog-meta">{formatDate(ver.date)}</span>
							{#if hasReal}
								<span class="alog-badge alog-badge--real">jury assessed</span>
							{:else if hasSynthetic}
								<span class="alog-badge alog-badge--synthetic">heuristic only *</span>
							{/if}
							<span class="alog-toggle">{assessmentLogExpanded === ver.version ? '▲' : '▼'}</span>
						</button>
						{#if assessmentLogExpanded === ver.version}
							<div class="alog-body">
								{#each iterProjects as project, pi}
									<div class="alog-project" style="border-left-color:hsl(var(--{chartColors[vi % chartColors.length]}))">
								...
								{/each}
								{#if (resultsMap[ver.version] ?? []).length > 10}
									<p class="alog-more">Showing top 10 of {(resultsMap[ver.version] ?? []).length} projects in this iteration</p>
								{/if}
								{#if hasSynthetic}
